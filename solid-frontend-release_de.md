...
3. PR erstellen für `dev` -> `master` und mergen.
   - Merge-Konflikte notfalls per rebase oder `git merge -s ours master` lösen
5. Einen neuen Release erstellen (Schaltfläche Release  --> `Draft a new Release`)
6. Eine Release-Message schreiben und die s.o.l.i.d.-Version mit angeben.
7. Im Web-Interface(!) einen _Tag_ auf den Merge-Commit bzw. `master` erstellen im Format `<appname>-v.X.Y.Z` (z.B. `dive-v.X.Y.Z`)
